...
//--------------------------------------------ultrasonic-arm-program-methods-------------------------------
void straightGripper(){
...
void pushButtonCallBack(const std_msgs::Bool::ConstPtr& msg)
{ 
  if(msg->data){
   button_pressed = true;
    openGripper();
...
void ultrasonicCallback (const sensor_msgs::Range::ConstPtr& range_data)
{
...
//---------------------------------     readTagCallback Function ------------------------------

void readTagCallback (const std_msgs::String& tagData)
{
    readTag=tagData;
  
   if(readTag.data.compare("3700161253")==0)  
     closeGripper();
	else
	openGripper();
}
...
//subscriber for Arbotix arm
	...
	//	ros::Subscriber armCommandsSubscriber = n.subscribe("arm_controller/command", 1, onArmCommand); // subscribing for joint states
		ros::Subscriber commandReceiver = n.subscribe("/arm_position_topic", 1000, armCallBack); // CallBack for arm controller
		ros::Subscriber readTag_sub = n.subscribe("/tagRead", 1000, readTagCallback);

		ros::Subscriber sensor_sub = n.subscribe("/ultrasound", 1000, ultrasonicCallback);
		ros::Subscriber button_sub = n.subscribe("/buttonPushed", 1000, pushButtonCallBack);
